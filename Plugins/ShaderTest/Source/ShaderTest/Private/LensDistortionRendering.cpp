...
/**
 * Internal intermediary structure derived from FShaderTestCameraModel by the game thread
...
struct FCompiledCameraModel
{
	...
	FShaderTestCameraModel OriginalCameraModel;
...
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationVS, TEXT("/Plugin/ShaderTest/Private/ShaderTest.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationPS, TEXT("/Plugin/ShaderTest/Private/ShaderTest.usf"), TEXT("MainPS"), SF_Pixel)
...
FVector2D FShaderTestCameraModel::UndistortNormalizedViewPosition(FVector2D EngineV) const
{
...
float FShaderTestCameraModel::GetUndistortOverscanFactor(
	float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
	...
	if (*this == FShaderTestCameraModel())
	{
		return 1.0f;
	}
...
void FShaderTestCameraModel::DrawUVDisplacementToRenderTarget(
	UWorld* World,
